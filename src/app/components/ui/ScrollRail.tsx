import { ReactNode } from "react";
import { useAppSettings } from "../../context/AppSettingsContext";

interface ScrollRailProps {
  children: ReactNode;
  itemWidth?: string;
  gap?: string;
  className?: string;
  /** Show edge fade gradients to hint scrollable content */
  showFades?: boolean;
}

export function ScrollRail({ children, gap = "0.5rem", className = "", showFades = true }: ScrollRailProps) {
  const { isRTL } = useAppSettings();

  return (
    <div className={`relative ${className}`}>
      {/* Edge fade — start & end indicators */}
      {showFades && (
        <>
          <div 
            className="absolute top-0 bottom-0 start-0 w-4 sm:w-7 z-10 pointer-events-none" 
            style={{ background: `linear-gradient(to ${isRTL ? "left" : "right"}, var(--color-background), transparent)` }} 
          />
          <div 
            className="absolute top-0 bottom-0 end-0 w-5 sm:w-8 z-10 pointer-events-none" 
            style={{ background: `linear-gradient(to ${isRTL ? "right" : "left"}, var(--color-background), transparent)` }} 
          />
        </>
      )}
      <div
        className="-mx-4 px-4 sm:mx-0 sm:px-0 overflow-x-auto scrollbar-hide snap-x snap-mandatory"
        style={{ WebkitOverflowScrolling: "touch" } as React.CSSProperties}
      >
        <div className="flex" style={{ gap }}>
          {children}
        </div>
      </div>
    </div>
  );
}
